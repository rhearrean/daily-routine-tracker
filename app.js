const APP_META={version:"11.1.0",build:"2026.08.22.repeatable-habits",schemaVersion:7,releaseDate:"August 22, 2026",releaseNotes:["Adds repeatable habits that can be logged multiple times inside each time-block occurrence.","Repeatable habits support an optional target while remaining available beyond the target.","Each occurrence keeps its own count, with +1 and undo controls.","Routine steps reset after each repeat is logged so they are ready for the next round."]};
...
function render(){renderTimeBlocks();renderSkipReview();renderEndOfDay();const streak=getOverallCurrentStreak();E.streakText.textContent=`${streak} ${streak===1?"day":"days"}`;const week=getWeekCount();E.weekText.textContent=`${week.completed}/${week.daysWithHabits||7}`;renderRecentDays();renderAllHabits();renderHabitStats();renderBlockSettings();renderOccurrenceButtons();renderDisplaySettings();renderAppInfo()}
