const APP_META={version:"10.8.2",build:"2026.07.01.ui-polish",schemaVersion:6,releaseDate:"June 28, 2026",releaseNotes:["UI polish: current block gets a green Current badge.","Future and completed blocks use compact one-line summaries.","Tap compact blocks to expand without changing data."]};
...
function blockSummaryText(completed,skipped,eligible){const parts=[];if(completed)parts.push(`${completed} completed`);if(skipped)parts.push(`${skipped} skipped`);if(!parts.length&&eligible===0)parts.push("All skipped");return parts.join(" • ")}function renderTimeBlocks(){
  ...
  orderedTodayBlocks().forEach(block=>{
    ...
    const shouldStartCompact=!isAnytime&&!isCurrent;
    const summary=compactBlockSummary(completed,eligible,skipped);

    const w=document.createElement("div");
    w.className=`time-block ${isAnytime?"anytime-block":""} ${isCurrent?"current-block":""} ${isComplete?"done-block":""} ${shouldStartCompact?"compact-block":""}`;
    ...
    ${shouldStartCompact?"":`<small class="block-time">${blockTimeLabel(block)}</small><div class="habit-list"></div>`}`;
