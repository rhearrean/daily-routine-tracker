const APP_META={version:"11.1.6",build:"2026.08.24.today-only-block-switch",schemaVersion:7,releaseDate:"August 24, 2026",releaseNotes:["Adds a Today-only switch between two time blocks.","Temporarily replaces the normally scheduled block and shows the replacement block's habits.","The normal block-day schedule returns automatically tomorrow.","The switch can be cleared early without losing completion history.","No data-schema changes."]};
...
const $=id=>document.getElementById(id);let selectedCustomDays=[],selectedOccurrenceBlocks=[],selectedRoutineSteps=[],selectedBlockDays=[0,1,2,3,4,5,6],editingHabitId=null,editingBlockId=null,pendingDeleteBlockId=null,activeAction=null,expandedCompletedBlocks={},expandedRoutineSteps={},skipReviewExpanded=false,endOfDayRoutineExpanded=false,recoveryTimer=null,waitingServiceWorker=null,updateReadyShown=false;
const E={todayTitle:$("todayTitle"),dateText:$("dateText"),mainVersionText:$("mainVersionText"),headerVersionBadge:$("headerVersionBadge"),progressText:$("progressText"),progressPercent:$("progressPercent"),timeBlocks:$("timeBlocks"),emptyTodayText:$("emptyTodayText"),todayRoutineCard:$("todayRoutineCard"),endOfDayCard:$("endOfDayCard"),endOfDayMessage:$("endOfDayMessage"),endOfDaySummary:$("endOfDaySummary"),endOfDayReviewBtn:$("endOfDayReviewBtn"),endOfDayRoutineBtn:$("endOfDayRoutineBtn"),resetTodayBtn:$("resetTodayBtn"),streakText:$("streakText"),weekText:$("weekText"),recentDays:$("recentDays"),habitForm:$("habitForm"),habitName:$("habitName"),habitSchedule:$("habitSchedule"),customDays:$("customDays"),habitRepeatable:$("habitRepeatable"),repeatTargetWrap:$("repeatTargetWrap"),habitRepeatTarget:$("habitRepeatTarget"),allHabits:$("allHabits"),habitStats:$("habitStats"),openAddHabitBtn:$("openAddHabitBtn"),openStatsBtn:$("openStatsBtn"),closeStatsBtn:$("closeStatsBtn"),statsPanel:$("statsPanel"),openSettingsBtn:$("openSettingsBtn"),closeSettingsBtn:$("closeSettingsBtn"),habitEditorSheet:$("habitEditorSheet"),closeHabitEditorBtn:$("closeHabitEditorBtn"),settingsPanel:$("settingsPanel"),exportBtn:$("exportBtn"),importBtn:$("importBtn"),backupBox:$("backupBox"),backupText:$("backupBox"),backupMessage:$("backupMessage"),recoveryStatus:$("recoveryStatus"),restoreRecoveryBtn:$("restoreRecoveryBtn"),updateSheet:$("updateSheet"),updateExportBtn:$("updateExportBtn"),updateBackupConfirmed:$("updateBackupConfirmed"),installUpdateBtn:$("installUpdateBtn"),laterUpdateBtn:$("laterUpdateBtn"),updateStatus:$("updateStatus"),updateReleaseSummary:$("updateReleaseSummary"),updateReleaseVersion:$("updateReleaseVersion"),updateReleaseNotes:$("updateReleaseNotes"),formTitle:$("formTitle"),formModeLabel:$("formModeLabel"),saveHabitBtn:$("saveHabitBtn"),cancelEditBtn:$("cancelEditBtn"),habitCutoff:$("habitCutoff"),habitAllowLate:$("habitAllowLate"),occurrenceBlocks:$("occurrenceBlocks"),blockSettings:$("blockSettings"),saveBlocksBtn:$("saveBlocksBtn"),appInfo:$("appInfo"),actionSheet:$("actionSheet"),actionTitle:$("actionTitle"),actionCompleteBtn:$("actionCompleteBtn"),actionSkipBtn:$("actionSkipBtn"),actionClearBtn:$("actionClearBtn"),actionCancelBtn:$("actionCancelBtn"),autoCollapseBlocks:$("autoCollapseBlocks"),skipReviewCard:$("skipReviewCard"),skipReviewToggle:$("skipReviewToggle"),skipReviewCount:$("skipReviewCount"),skipReviewMessage:$("skipReviewMessage"),skipReviewChevron:$("skipReviewChevron"),skipReviewDetails:$("skipReviewDetails"),addBlockBtn:$("addBlockBtn"),blockEditorSheet:$("blockEditorSheet"),blockForm:$("blockForm"),blockFormModeLabel:$("blockFormModeLabel"),blockFormTitle:$("blockFormTitle"),blockName:$("blockName"),blockStart:$("blockStart"),blockEnd:$("blockEnd"),blockTimeFields:$("blockTimeFields"),blockConflictMessage:$("blockConflictMessage"),saveBlockBtn:$("saveBlockBtn"),closeBlockEditorBtn:$("closeBlockEditorBtn"),cancelBlockEditBtn:$("cancelBlockEditBtn"),blockActiveDaysWrap:$("blockActiveDaysWrap"),blockActiveDays:$("blockActiveDays"),settingsAddHabitBtn:$("settingsAddHabitBtn"),deleteBlockBtn:$("deleteBlockBtn"),blockDeleteSheet:$("blockDeleteSheet"),blockDeleteTitle:$("blockDeleteTitle"),blockDeleteSummary:$("blockDeleteSummary"),blockDeleteAffected:$("blockDeleteAffected"),blockDeleteMoveTarget:$("blockDeleteMoveTarget"),closeBlockDeleteBtn:$("closeBlockDeleteBtn"),cancelBlockDeleteBtn:$("cancelBlockDeleteBtn"),confirmMoveDeleteBtn:$("confirmMoveDeleteBtn"),confirmRemoveDeleteBtn:$("confirmRemoveDeleteBtn")};
const TODAY_BLOCK_SWITCH_UI={wrap:$("todayBlockSwitch"),from:$("todayBlockSwitchFrom"),to:$("todayBlockSwitchTo"),apply:$("applyTodayBlockSwitchBtn"),clear:$("clearTodayBlockSwitchBtn"),status:$("todayBlockSwitchStatus")};
...
function normalizeCompletionEntry(e){if(e===true)return{state:"done",done:true,status:"done",completedAt:""};if(e&&typeof e==="object"){const state=e.state||(e.skipped?"skipped":(e.done!==false?"done":"pending"));return{...e,state,done:state==="done",status:e.status||state,...(e.repeatCount===undefined?{}:{repeatCount:Math.max(0,Number(e.repeatCount)||0)}),repeatLog:Array.isArray(e.repeatLog)?e.repeatLog.map(String):[]}}return null}function normalizeHabit(h){const oldBlock=h.block||"anytime";let occ=Array.isArray(h.occurrences)&&h.occurrences.length>0?h.occurrences:[{id:`${h.id||makeId(h.name||"habit")}-${oldBlock}`,block:oldBlock}];occ=occ.map((o,i)=>({id:o.id||`${h.id||makeId(h.name||"habit")}-${o.block||oldBlock}-${i}`,block:o.block||oldBlock}));const routineSteps=Array.isArray(h.routineSteps)?h.routineSteps.map((s,i)=>typeof s==="string"?{id:`step-${i}-${makeId(s)}`,text:s}:{id:s.id||`step-${i}-${makeId(s.text||"step")}`,text:String(s.text||"").trim(),createdAt:String(s.createdAt||"")}).filter(s=>s.text):[],repeatTarget=Math.max(0,Math.floor(Number(h.repeatTarget)||0));return{id:h.id||makeId(h.name||"Habit"),name:h.name||"Habit",schedule:h.schedule||"daily",days:Array.isArray(h.days)?h.days:[],occurrences:sortOccurrences(occ),cutoff:h.cutoff||"",allowLate:h.allowLate!==false,snoozeUntil:h.snoozeUntil||"",paused:h.paused===true,pausePeriods:Array.isArray(h.pausePeriods)?h.pausePeriods.map(period=>({start:String(period.start||""),end:String(period.end||"")})).filter(period=>period.start):[],repeatable:h.repeatable===true,repeatTarget,routineSteps}}
...
function routineStepsForOccurrence(h,dateKey,occurrenceId){
  const steps=h.routineSteps||[],entry=completionEntry(dateKey,occurrenceId);
  if(h.repeatable)return steps;
  if(entry?.state!=="done")return steps;
  if(!entry.completedAt)return steps.filter(step=>!step.createdAt);
  return steps.filter(step=>!step.createdAt||step.createdAt<=entry.completedAt);
}
function routineStepProgress(h,dateKey,occurrenceId){
  const steps=routineStepsForOccurrence(h,dateKey,occurrenceId),done=steps.filter(s=>isRoutineStepDone(dateKey,occurrenceId,s.id)).length;
  return{done,total:steps.length};
}
function moveRoutineStep(index,direction){
  const nextIndex=index+direction;
  if(nextIndex<0||nextIndex>=selectedRoutineSteps.length)return;
  [selectedRoutineSteps[index],selectedRoutineSteps[nextIndex]]=[selectedRoutineSteps[nextIndex],selectedRoutineSteps[index]];
  renderRoutineStepsEditor();
  E.routineStepsList.querySelectorAll("input")[nextIndex]?.focus();
}
function renderRoutineStepsEditor(){
  ...
  selectedRoutineSteps.forEach((step,index)=>{
    ...
    row.innerHTML=`<input type="text" value="${String(step.text).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}" aria-label="Routine step ${index+1}"/><div class="routine-step-reorder"><button type="button" class="reorder-btn routine-step-up" aria-label="Move step ${index+1} up" ${index===0?"disabled":""}>↑</button><button type="button" class="reorder-btn routine-step-down" aria-label="Move step ${index+1} down" ${index===selectedRoutineSteps.length-1?"disabled":""}>↓</button></div><button type="button" class="danger-btn remove-step-btn" aria-label="Remove step">✕</button>`;
    row.querySelector("input").addEventListener("input",e=>{selectedRoutineSteps[index].text=e.target.value});
    row.querySelector(".routine-step-up").addEventListener("click",()=>moveRoutineStep(index,-1));
    row.querySelector(".routine-step-down").addEventListener("click",()=>moveRoutineStep(index,1));
...
function addRoutineStep(){
  selectedRoutineSteps.push({id:`step-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,text:"",createdAt:new Date().toISOString()});
  renderRoutineStepsEditor();
...
function isHabitPausedOn(h,d=new Date()){
  const dateKey=getLocalDateKey(d),periods=Array.isArray(h.pausePeriods)?h.pausePeriods:[];
  if(h.paused&&periods.length===0)return true;
  return periods.some(period=>period.start&&dateKey>=period.start&&(!period.end||dateKey<period.end));
}
...
function isHabitDue(h,d){if(isHabitPausedOn(h,d)||isHabitSnoozed(h,d))return false;const day=d.getDay();if(h.schedule==="daily")return true;if(h.schedule==="weekdays")return day>=1&&day<=5;if(h.schedule==="weekends")return day===0||day===6;if(h.schedule==="custom")return h.days.includes(day);return true}function dueOccurrencesOn(d){return loadHabits().filter(h=>isHabitDue(h,d)).flatMap(h=>h.occurrences.filter(o=>blockIsActiveOn(blockById(o.block),d)).map(o=>({habit:h,occurrence:o})))}function completionEntry(k,id){return normalizeCompletionEntry(loadCompletions()[k]?.[id])}function stateOf(k,id){return completionEntry(k,id)?.state||"pending"}function habitForOccurrence(id){return loadHabits().find(h=>h.occurrences.some(o=>o.id===id))}function repeatCountFor(k,id){const entry=completionEntry(k,id);if(!entry)return 0;if(entry.repeatCount!==undefined)return Math.max(0,Math.floor(Number(entry.repeatCount)||0));return entry.state==="done"?1:0}function repeatTargetFor(h){return Math.max(0,Math.floor(Number(h?.repeatTarget)||0))}function isDone(k,id){const h=habitForOccurrence(id);if(h?.repeatable&&!isSkipped(k,id))return repeatCountFor(k,id)>=(repeatTargetFor(h)||1);return stateOf(k,id)==="done"}function isSkipped(k,id){return stateOf(k,id)==="skipped"}function isResolved(k,id){return isDone(k,id)||isSkipped(k,id)}function completionStatus(k,id){return completionEntry(k,id)?.status||""}function isPastCutoff(h){const c=timeToMinutes(h.cutoff);return c!==null&&nowMinutes()>c}function getCurrentCompletionStatus(h){return!h.cutoff?"done":isPastCutoff(h)?"late":"done"}
function addRepeatCompletion(k,h,o){
  if(!h.repeatable)return setOccurrenceState(k,h,o,"done");
  const s=getCurrentCompletionStatus(h);
  if(s==="late"&&h.allowLate===false){alert("This occurrence is past its cutoff time.");return}
  const c=loadCompletions(),existing=normalizeCompletionEntry(c[k]?.[o.id])||{},now=new Date().toISOString();
  c[k]=c[k]||{};
  const repeatCount=repeatCountFor(k,o.id)+1,repeatTarget=repeatTargetFor(h),done=repeatCount>=(repeatTarget||1);
  c[k][o.id]={...existing,state:done?"done":"pending",done,status:s,completedAt:done?(existing.completedAt||now):"",habitId:h.id,occurrenceId:o.id,block:o.block,repeatCount,repeatLog:[...(existing.repeatLog||[]),now]};
  if(Array.isArray(h.routineSteps)&&h.routineSteps.length)clearRoutineStepsForOccurrence(k,o.id);
  if(done)delete expandedRoutineSteps[o.id];
  saveCompletions(c);
  render();
}
function removeRepeatCompletion(k,h,o){
  if(!h.repeatable)return;
  const c=loadCompletions(),existing=normalizeCompletionEntry(c[k]?.[o.id]);
  if(!existing||isSkipped(k,o.id))return;
  const repeatCount=Math.max(0,repeatCountFor(k,o.id)-1);
  if(repeatCount===0){delete c[k][o.id]}
  else{
    const repeatTarget=repeatTargetFor(h),done=repeatCount>=(repeatTarget||1),next={...existing,state:done?"done":"pending",done,repeatCount,repeatLog:(existing.repeatLog||[]).slice(0,-1)};
    if(done)next.completedAt=existing.completedAt||new Date().toISOString();else delete next.completedAt;
    c[k][o.id]=next;
  }
  saveCompletions(c);
  render();
}
function setOccurrenceState(k,h,o,state){
  if(state==="done"&&h.repeatable){addRepeatCompletion(k,h,o);return}
  if(state==="skipped"&&h.repeatable&&isDone(k,o.id))return;
  const c=loadCompletions();
  c[k]=c[k]||{};
  if(state==="clear")delete c[k][o.id];
  else if(state==="skipped"){const existing=normalizeCompletionEntry(c[k][o.id])||{};c[k][o.id]={...existing,state:"skipped",done:false,status:"skipped",skippedAt:new Date().toISOString(),habitId:h.id,occurrenceId:o.id,block:o.block}}
  else{
    const s=getCurrentCompletionStatus(h);
    if(s==="late"&&h.allowLate===false){alert("This occurrence is past its cutoff time.");return}
    c[k][o.id]={state:"done",done:true,status:s,completedAt:new Date().toISOString(),habitId:h.id,occurrenceId:o.id,block:o.block};
    if(Array.isArray(h.routineSteps)&&h.routineSteps.length)delete expandedRoutineSteps[o.id];
  }
  saveCompletions(c);
  render();
}
function toggleDone(k,h,o){if(h.repeatable){addRepeatCompletion(k,h,o);return}isResolved(k,o.id)?setOccurrenceState(k,h,o,"clear"):setOccurrenceState(k,h,o,"done")}function openActionMenu(k,h,o){activeAction={dateKey:k,habit:h,occurrence:o};E.actionTitle.textContent=h.name;E.actionSheet.classList.remove("hidden")}function closeActionMenu(){activeAction=null;E.actionSheet.classList.add("hidden")}function resetToday(){skipReviewExpanded=false;const k=getTodayKey(),c=loadCompletions();delete c[k];saveCompletions(c);const steps=loadRoutineStepState();delete steps[k];saveRoutineStepState(steps);expandedRoutineSteps={};render()}
...
function timeBlockPhaseSignature(){return orderedTodayBlocks().map(block=>`${block.id}:${block.id==="anytime"?"anytime":isCurrentTimedBlock(block)?"current":isPastTimedBlock(block)?"past":"future"}`).join("|")}
let lastTimeBlockPhaseSignature="";
function refreshTimeBlockPhase(){const next=timeBlockPhaseSignature();if(next===lastTimeBlockPhaseSignature)return;lastTimeBlockPhaseSignature=next;expandedCompletedBlocks={};render()}
...
function renderEndOfDay(){
  if(!E.endOfDayCard||!E.todayRoutineCard)return;
  const due=dueOccurrencesOn(new Date()),dateKey=getTodayKey();
  const finished=due.length>0&&due.every(({occurrence})=>isResolved(dateKey,occurrence.id));
  const completed=due.filter(({occurrence})=>isDone(dateKey,occurrence.id)).length;
  const skipped=due.filter(({occurrence})=>isSkipped(dateKey,occurrence.id)).length;

  if(!finished){
    endOfDayRoutineExpanded=false;
    E.endOfDayCard.classList.add("hidden");
    E.todayRoutineCard.classList.remove("hidden");
    E.skipReviewCard.classList.remove("hidden");
    return;
  }

  E.endOfDayCard.classList.remove("hidden");
  E.todayRoutineCard.classList.toggle("hidden",!endOfDayRoutineExpanded);
  E.skipReviewCard.classList.toggle("hidden",skipped===0||!skipReviewExpanded);
  E.endOfDayMessage.textContent="You're all done for today.";
  E.endOfDaySummary.textContent=skipped===0
    ?`All ${completed} scheduled ${completed===1?"habit is":"habits are"} complete.`
    :`${completed} completed · ${skipped} skipped`;
  E.endOfDayReviewBtn.classList.toggle("hidden",skipped===0);
  E.endOfDayReviewBtn.textContent=skipReviewExpanded?"Hide Skipped":"Review Skipped";
  E.endOfDayRoutineBtn.textContent=endOfDayRoutineExpanded?"Hide Today's Routine":"Show Today's Routine";
}

function blockSummaryText(completed,skipped,eligible){const total=eligible+skipped;let text=`${completed}/${total}`;if(skipped)text+=` · ${skipped} skip`;return text}function renderTimeBlocks(){E.timeBlocks.innerHTML="";const due=dueOccurrencesOn(new Date()),k=getTodayKey(),settings=loadSettings();E.emptyTodayText.classList.toggle("hidden",due.length>0);orderedTodayBlocks().forEach(block=>{const items=due.filter(i=>i.occurrence.block===block.id);if(!items.length)return;const completed=items.filter(i=>isDone(k,i.occurrence.id)).length,skipped=items.filter(i=>isSkipped(k,i.occurrence.id)).length,eligible=items.length-skipped,remaining=Math.max(eligible-completed,0),blockComplete=items.length>0&&remaining===0,autoCollapse=settings.autoCollapseCompletedBlocks!==false,defaultCollapsed=autoCollapse&&shouldCollapseBlock(block,blockComplete),manualCollapsed=expandedCompletedBlocks[block.id],collapsed=manualCollapsed===undefined?defaultCollapsed:manualCollapsed,w=document.createElement("div");w.className=`time-block ${collapsed?"collapsed":""} ${blockComplete?"completed-block":""} ${isCurrentTimedBlock(block)?"current-block":""}`;const summary=blockComplete?blockSummaryText(completed,skipped,eligible):`${remaining} remaining`;w.innerHTML=`<div class="time-block-header"><div><h3>${collapsed?"▶":"▼"} ${block.label}${blockComplete?" ✓":""}</h3><small>${blockTimeLabel(block)} ${isCurrentTimedBlock(block)?`<span class="current-badge"><span class="current-dot"></span>Current</span>`:""}</small>${collapsed?`<div class="block-summary">${summary}</div>`:""}</div><span class="block-progress">${blockComplete?summary:`${completed}/${eligible}${skipped?` • ${skipped} skipped`:""}`}</span></div><div class="habit-list"></div>`;w.querySelector(".time-block-header").addEventListener("click",e=>{if(e.target.closest("button"))return;expandedCompletedBlocks[block.id]=!collapsed;render()});const list=w.querySelector(".habit-list");items.forEach(({habit,occurrence})=>{const repeatable=habit.repeatable===true,repeatCount=repeatCountFor(k,occurrence.id),repeatTarget=repeatTargetFor(habit),done=isDone(k,occurrence.id),skipped=isSkipped(k,occurrence.id),status=completionStatus(k,occurrence.id),locked=!done&&!skipped&&habit.allowLate===false&&isPastCutoff(habit),occurrenceSteps=routineStepsForOccurrence(habit,k,occurrence.id),hasSteps=occurrenceSteps.length>0,expanded=!!expandedRoutineSteps[occurrence.id],progress=routineStepProgress(habit,k,occurrence.id),card=document.createElement("div");card.className=`habit-with-steps habit-card-shell ${hasSteps?"has-routine-steps":""} ${repeatable?"repeatable-habit":""} ${expanded?"steps-expanded":""}`;const row=document.createElement("div");row.className="habit-card-row";const b=document.createElement("button");b.className=`habit-button ${done?(status==="late"?"late":"done"):""} ${skipped?"skipped":""} ${locked?"locked":""}`;b.type="button";const sorted=sortOccurrences(habit.occurrences),num=habit.occurrences.length>1?` (${sorted.findIndex(o=>o.id===occurrence.id)+1} of ${habit.occurrences.length})`:"",repeatText=repeatTarget?`${repeatCount} / ${repeatTarget}`:`${repeatCount} ${repeatCount===1?"time":"times"}`;const pill=skipped?`<span class="status-pill skipped">Skipped</span>`:locked?`<span class="status-pill locked">Missed cutoff</span>`:repeatable?`<span class="status-pill repeat-count-pill">${repeatText}${done?" ✓":""}</span>`:done&&status==="late"&&habit.cutoff?`<span class="status-pill late">Late</span>`:hasSteps?`<span class="status-pill routine-step-pill">${progress.done}/${progress.total} steps • ${expanded?"hide":"show"}</span>`:!done&&habit.cutoff?`<span class="status-pill">Target: ${formatTime(habit.cutoff)}</span>`:"";b.innerHTML=`<span class="checkbox"></span><span class="habit-button-content"><strong>${habit.name}${num}</strong>${pill}</span>${hasSteps?`<span class="steps-chevron">${expanded?"▲":"▼"}</span>`:""}`;b.addEventListener("click",e=>{e.stopPropagation();if(hasSteps){expandedRoutineSteps[occurrence.id]=!expanded;render()}else if(repeatable&&!skipped&&!locked)addRepeatCompletion(k,habit,occurrence);else toggleDone(k,habit,occurrence)});row.appendChild(b);if(!repeatable&&!done&&!skipped&&!locked){const skipBtn=document.createElement("button");skipBtn.type="button";skipBtn.className="habit-skip-btn";skipBtn.textContent="Skip";skipBtn.setAttribute("aria-label",`Skip ${habit.name}`);skipBtn.addEventListener("click",e=>{e.stopPropagation();setOccurrenceState(k,habit,occurrence,"skipped")});row.appendChild(skipBtn)}card.appendChild(row);if(repeatable&&!skipped&&!locked){const repeatActions=document.createElement("div");repeatActions.className="repeat-actions";repeatActions.innerHTML=`<button type="button" class="small-btn repeat-undo-btn" ${repeatCount===0?"disabled":""}>−1 Undo</button><button type="button" class="primary-btn inline repeat-add-btn">＋1 Time</button><button type="button" class="habit-skip-btn repeat-skip-btn">Skip</button>`;repeatActions.querySelector(".repeat-undo-btn").addEventListener("click",()=>removeRepeatCompletion(k,habit,occurrence));repeatActions.querySelector(".repeat-add-btn").addEventListener("click",()=>addRepeatCompletion(k,habit,occurrence));repeatActions.querySelector(".repeat-skip-btn").addEventListener("click",()=>setOccurrenceState(k,habit,occurrence,"skipped"));card.appendChild(repeatActions)}if(hasSteps&&expanded){const panel=document.createElement("div");panel.className="routine-steps-panel";occurrenceSteps.forEach(step=>{const label=document.createElement("label");label.className="routine-step-check";label.innerHTML=`<input type="checkbox" ${isRoutineStepDone(k,occurrence.id,step.id)?"checked":""}/><span>${String(step.text).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}</span>`;label.querySelector("input").addEventListener("change",()=>toggleRoutineStep(k,occurrence.id,step.id));panel.appendChild(label)});const actions=document.createElement("div");actions.className="routine-step-actions";actions.innerHTML=`<span class="routine-step-count">${progress.done}/${progress.total} complete</span><button type="button" class="primary-btn inline routine-complete-btn">${repeatable?"Log 1 Time":done?"Clear Habit":"Complete Habit"}</button>`;actions.querySelector(".routine-complete-btn").addEventListener("click",()=>repeatable?addRepeatCompletion(k,habit,occurrence):toggleDone(k,habit,occurrence));panel.appendChild(actions);card.appendChild(panel)}list.appendChild(card)});E.timeBlocks.appendChild(w)})}
...
function renderAllHabits(){E.allHabits.innerHTML="";const habits=loadHabits();if(!habits.length){E.allHabits.innerHTML=`<p class="empty-text">No habits yet. Add one from Today.</p>`;return}habits.forEach((h,index)=>{const paused=h.paused===true,snoozed=isHabitSnoozed(h),inactive=paused||snoozed,row=document.createElement("div");row.className=`habit-row compact-habit-row ${inactive?"snoozed-habit":""}`;const blocks=[...new Set(h.occurrences.map(o=>blockById(o.block).label))];let blockSummary="No block";if(blocks.length===1)blockSummary=blocks[0];else if(blocks.length<=3)blockSummary=blocks.join(" • ");else blockSummary=`${blocks.length} blocks`;row.innerHTML=`<div class="habit-row-main"><div class="compact-habit-title"><strong>${h.name}</strong>${inactive?`<span class="habit-status-pill">${paused?"Paused":`Until ${snoozeLabel(h)}`}</span>`:""}</div><small>${blockSummary}</small></div><div class="habit-actions compact-habit-actions"><div class="reorder-actions"><button class="reorder-btn move-up-btn" type="button" aria-label="Move ${h.name} up" ${index===0?"disabled":""}>↑</button><button class="reorder-btn move-down-btn" type="button" aria-label="Move ${h.name} down" ${index===habits.length-1?"disabled":""}>↓</button></div><button class="small-btn resume-btn ${inactive?"":"pause-btn"}" type="button" aria-label="${inactive?"Resume":"Pause"} ${h.name}">${inactive?"Resume":"Pause"}</button><button class="edit-btn" type="button">Edit</button><button class="danger-btn compact-delete-btn" type="button" aria-label="Delete ${h.name}">✕</button></div>`;row.querySelector(".move-up-btn").addEventListener("click",()=>moveHabit(h.id,-1));row.querySelector(".move-down-btn").addEventListener("click",()=>moveHabit(h.id,1));const pauseToggle=row.querySelector(".resume-btn");pauseToggle.addEventListener("click",()=>inactive?resumeHabit(h.id):pauseHabit(h.id));row.querySelector(".edit-btn").addEventListener("click",()=>startEditHabit(h.id));row.querySelector(".compact-delete-btn").addEventListener("click",()=>{if(confirm(`Delete "${h.name}"?`))deleteHabit(h.id)});E.allHabits.appendChild(row)})}function renderHabitStats(){E.habitStats.innerHTML="";const habits=loadHabits();if(!habits.length){E.habitStats.innerHTML=`<p class="empty-text">No habit stats yet.</p>`;return}habits.forEach(h=>{const s=getHabitStats(h),card=document.createElement("div");card.className="habit-stat-card";card.innerHTML=`<strong>${s.name}</strong><small>${s.occurrences.map(o=>o.label).join(", ")} • ${s.schedule} • ${s.cutoff?formatTime(s.cutoff):"No cutoff"}</small><div class="metric-row"><div class="metric"><b>${s.currentStreak}</b><span>Current</span></div><div class="metric"><b>${s.longestStreak}</b><span>Longest</span></div><div class="metric"><b>${s.last30Days.percent}%</b><span>30 Days</span></div><div class="metric"><b>${s.totalSkipped}</b><span>Skipped</span></div><div class="metric"><b>${s.totalLate}</b><span>Late</span></div><div class="metric"><b>${s.totalCompleted}</b><span>Total</span></div></div>`;E.habitStats.appendChild(card)})}
const BLOCK_DAY_LABELS=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];
function blockScheduledOn(block,date=new Date()){return block.id==="anytime"||!Array.isArray(block.days)||block.days.includes(date.getDay())}
function activeTodayBlockSwitch(date=new Date()){const value=loadSettings().todayBlockSwitch;return value&&value.date===getLocalDateKey(date)&&value.fromBlockId&&value.toBlockId&&value.fromBlockId!==value.toBlockId?value:null}
function blockIsActiveOn(block,date=new Date()){const scheduled=blockScheduledOn(block,date),todaySwitch=activeTodayBlockSwitch(date);if(!todaySwitch||block.id==="anytime")return scheduled;if(block.id===todaySwitch.fromBlockId)return false;if(block.id===todaySwitch.toBlockId)return true;return scheduled}
...
function switchableTimeBlocks(){return loadBlocks().filter(block=>block.id!=="anytime")}
function setTodayBlockSwitchOptions(select,blocks,selected){select.innerHTML="";blocks.forEach(block=>{const option=document.createElement("option");option.value=block.id;option.textContent=block.label;select.appendChild(option)});if(selected&&blocks.some(block=>block.id===selected))select.value=selected}
function renderTodayBlockSwitchTargets(selected=""){const ui=TODAY_BLOCK_SWITCH_UI;if(!ui.to)return;const blocks=switchableTimeBlocks().filter(block=>block.id!==ui.from.value);setTodayBlockSwitchOptions(ui.to,blocks,selected);ui.apply.disabled=!ui.from.value||!ui.to.value}
function renderTodayBlockSwitch(){const ui=TODAY_BLOCK_SWITCH_UI;if(!ui.wrap)return;const blocks=switchableTimeBlocks(),today=new Date(),current=activeTodayBlockSwitch(today),scheduled=blocks.filter(block=>blockScheduledOn(block,today));setTodayBlockSwitchOptions(ui.from,scheduled,current?.fromBlockId||scheduled[0]?.id||"");renderTodayBlockSwitchTargets(current?.toBlockId||"");ui.clear.classList.toggle("hidden",!current);if(current){ui.status.textContent=`Today: ${blockLabel(current.fromBlockId)} → ${blockLabel(current.toBlockId)}`;ui.status.classList.add("active")}else{ui.status.textContent=blocks.length<2?"Add at least two timed blocks to use a Today-only switch.":scheduled.length?"No temporary switch is active.":"No timed block is normally scheduled today.";ui.status.classList.remove("active")}ui.from.disabled=scheduled.length===0;ui.to.disabled=blocks.length<2||scheduled.length===0;ui.apply.disabled=!ui.from.value||!ui.to.value}
function applyTodayBlockSwitch(){const ui=TODAY_BLOCK_SWITCH_UI,fromBlockId=ui.from.value,toBlockId=ui.to.value;if(!fromBlockId||!toBlockId||fromBlockId===toBlockId)return;const from=blockById(fromBlockId),to=blockById(toBlockId);if(!from||!to)return;if(!confirm(`Use "${to.label}" instead of "${from.label}" for Today only?\n\nYour normal schedule returns tomorrow.`))return;saveSettings({...loadSettings(),todayBlockSwitch:{date:getTodayKey(),fromBlockId,toBlockId}});expandedCompletedBlocks={};endOfDayRoutineExpanded=false;lastTimeBlockPhaseSignature=timeBlockPhaseSignature();render()}
function clearTodayBlockSwitch(){const settings=loadSettings();if(!settings.todayBlockSwitch)return;delete settings.todayBlockSwitch;saveSettings(settings);expandedCompletedBlocks={};endOfDayRoutineExpanded=false;lastTimeBlockPhaseSignature=timeBlockPhaseSignature();render()}
function clearExpiredTodayBlockSwitch(){const settings=loadSettings();if(!settings.todayBlockSwitch||settings.todayBlockSwitch.date===getTodayKey())return false;delete settings.todayBlockSwitch;saveSettings(settings);return true}
...
function showUpdateReleaseInfo(meta){
  if(!E.updateReleaseSummary||!E.updateReleaseVersion||!E.updateReleaseNotes)return;
  if(!meta){
  ...
  const notes=Array.isArray(meta.notes)&&meta.notes.length?meta.notes:[meta.summary||"This update includes routine improvements and fixes."];
  E.updateReleaseVersion.textContent=meta.version?`What's new in v${meta.version}`:"What's new";
  E.updateReleaseNotes.innerHTML=notes.map(note=>`<li>${String(note).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}</li>`).join("");
  E.updateReleaseSummary.classList.remove("hidden");
}
...
function setupSafeUpdateFlow(){
  ...
  navigator.serviceWorker.addEventListener("message",event=>{
    if(event.data?.type==="RELEASE_META")showUpdateReleaseInfo(event.data.meta||null);
  });
...
function simplifyRepeatControls(){document.querySelectorAll(".repeat-undo-btn,.has-routine-steps.repeatable-habit .repeat-add-btn").forEach(button=>button.remove());document.querySelectorAll(".repeatable-habit:not(.has-routine-steps) .habit-button").forEach(button=>{button.tabIndex=-1;button.setAttribute("aria-disabled","true")});document.querySelectorAll(".repeat-add-btn").forEach(button=>button.textContent="Log Completion");document.querySelectorAll(".repeatable-habit").forEach(card=>{const targetReached=card.querySelector(".habit-button.done .repeat-count-pill")?.textContent.includes("/");if(!targetReached)return;const habitButton=card.querySelector(".habit-button"),logButton=card.querySelector(".repeat-add-btn,.routine-complete-btn");if(habitButton){habitButton.disabled=true;habitButton.setAttribute("aria-disabled","true")}if(logButton){logButton.disabled=true;logButton.textContent="Target Complete"}});document.querySelectorAll(".repeat-actions").forEach(actions=>{if(!actions.children.length)actions.remove()})}
function render(){renderTimeBlocks();simplifyRepeatControls();renderSkipReview();renderEndOfDay();const streak=getOverallCurrentStreak();E.streakText.textContent=`${streak} ${streak===1?"day":"days"}`;const week=getWeekCount();E.weekText.textContent=`${week.completed}/${week.daysWithHabits||7}`;renderRecentDays();renderAllHabits();renderHabitStats();renderBlockSettings();renderTodayBlockSwitch();renderOccurrenceButtons();renderDisplaySettings();renderAppInfo()}
function setSelectedCustomDays(days){selectedCustomDays=[...days];E.customDays.querySelectorAll("button").forEach(b=>b.classList.toggle("selected",selectedCustomDays.includes(Number(b.dataset.day))))}function setSelectedOccurrenceBlocks(blocks){selectedOccurrenceBlocks=[...blocks];renderOccurrenceButtons()}function updateRepeatTargetVisibility(){E.repeatTargetWrap.classList.toggle("hidden",!E.habitRepeatable.checked)}function resetFormMode(){editingHabitId=null;selectedRoutineSteps=[];renderRoutineStepsEditor();E.habitName.value="";E.habitSchedule.value="daily";E.habitRepeatable.checked=false;E.habitRepeatTarget.value="";updateRepeatTargetVisibility();E.habitCutoff.value="";if(E.habitSnoozeUntil)E.habitSnoozeUntil.value="";E.habitAllowLate.checked=true;setSelectedCustomDays([]);setSelectedOccurrenceBlocks(["evening"]);E.customDays.classList.add("hidden");E.formModeLabel.textContent="Manage Habits";E.formTitle.textContent="Add Habit";E.saveHabitBtn.textContent="Add Habit";E.cancelEditBtn.classList.add("hidden")}function openHabitEditor(){E.habitEditorSheet.classList.remove("hidden");document.body.style.overflow="hidden"}function closeHabitEditor(){E.habitEditorSheet.classList.add("hidden");document.body.style.overflow=E.settingsPanel.classList.contains("hidden")?"":"hidden"}function openAddHabit(){resetFormMode();openHabitEditor()}function startEditHabit(id){const h=loadHabits().find(x=>x.id===id);if(!h)return;editingHabitId=id;E.habitName.value=h.name;E.habitSchedule.value=h.schedule;E.habitRepeatable.checked=h.repeatable===true;E.habitRepeatTarget.value=h.repeatTarget||"";updateRepeatTargetVisibility();E.habitCutoff.value=h.cutoff||"";if(E.habitSnoozeUntil)E.habitSnoozeUntil.value=h.snoozeUntil||"";E.habitAllowLate.checked=h.allowLate!==false;setSelectedCustomDays(Array.isArray(h.days)?h.days:[]);setSelectedOccurrenceBlocks(h.occurrences.map(o=>o.block));selectedRoutineSteps=(h.routineSteps||[]).map(s=>({...s}));renderRoutineStepsEditor();E.customDays.classList.toggle("hidden",h.schedule!=="custom");E.formModeLabel.textContent="Editing Habit";E.formTitle.textContent="Edit Habit";E.saveHabitBtn.textContent="Save Changes";E.cancelEditBtn.classList.remove("hidden");openHabitEditor()}function saveHabitFromForm(e){e.preventDefault();const name=E.habitName.value.trim(),schedule=E.habitSchedule.value,repeatable=E.habitRepeatable.checked,repeatTarget=repeatable?Math.max(0,Math.floor(Number(E.habitRepeatTarget.value)||0)):0;if(!name){alert("Add a habit name first.");return}if(schedule==="custom"&&selectedCustomDays.length===0){alert("Choose at least one custom day.");return}if(selectedOccurrenceBlocks.length===0){alert("Choose at least one occurrence block.");return}const habits=loadHabits(),oldHabit=editingHabitId?habits.find(h=>h.id===editingHabitId):null,habitId=editingHabitId||makeId(name),oldByBlock={};if(oldHabit)oldHabit.occurrences.forEach(o=>oldByBlock[o.block]=o.id);const occurrences=sortOccurrences(selectedOccurrenceBlocks.map((block,index)=>({id:oldByBlock[block]||`${habitId}-${block}-${Date.now()}-${index}`,block})));const routineSteps=selectedRoutineSteps.map(s=>({id:s.id||`step-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,text:String(s.text||"").trim(),createdAt:String(s.createdAt||"")})).filter(s=>s.text);const data={name,schedule,days:schedule==="custom"?[...selectedCustomDays].sort():[],occurrences,repeatable,repeatTarget,cutoff:E.habitCutoff.value,allowLate:E.habitAllowLate.checked,snoozeUntil:E.habitSnoozeUntil?E.habitSnoozeUntil.value:"",routineSteps};if(editingHabitId){const i=habits.findIndex(h=>h.id===editingHabitId);if(i>=0)habits[i]={...habits[i],...data}}else habits.push({id:habitId,...data});saveHabits(habits);resetFormMode();closeHabitEditor();render()}function saveBlockSettings(){}function openStats(){E.statsPanel.classList.remove("hidden");document.body.style.overflow="hidden"}function closeStats(){E.statsPanel.classList.add("hidden");document.body.style.overflow=""}function openSettings(){E.settingsPanel.classList.remove("hidden");document.body.style.overflow="hidden"}function closeSettings(){E.settingsPanel.classList.add("hidden");document.body.style.overflow=""}
...
E.habitSchedule.addEventListener("change",()=>E.customDays.classList.toggle("hidden",E.habitSchedule.value!=="custom"));E.habitRepeatable.addEventListener("change",updateRepeatTargetVisibility);E.customDays.querySelectorAll("button").forEach(b=>b.addEventListener("click",()=>{const day=Number(b.dataset.day);selectedCustomDays=selectedCustomDays.includes(day)?selectedCustomDays.filter(d=>d!==day):[...selectedCustomDays,day];b.classList.toggle("selected",selectedCustomDays.includes(day))}));E.habitForm.addEventListener("submit",saveHabitFromForm);E.cancelEditBtn.addEventListener("click",()=>{resetFormMode();closeHabitEditor()});E.resetTodayBtn.addEventListener("click",resetToday);
...
TODAY_BLOCK_SWITCH_UI.from.addEventListener("change",()=>renderTodayBlockSwitchTargets());
TODAY_BLOCK_SWITCH_UI.apply.addEventListener("click",applyTodayBlockSwitch);
TODAY_BLOCK_SWITCH_UI.clear.addEventListener("click",clearTodayBlockSwitch);
...
setupSnoozeField();setupRoutineStepsField();migrateOldDataOnce();clearExpiredSnoozes();clearExpiredTodayBlockSwitch();formatDateLabel();resetFormMode();render();lastTimeBlockPhaseSignature=timeBlockPhaseSignature();setInterval(refreshTimeBlockPhase,30000);document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="visible"){clearExpiredTodayBlockSwitch();refreshTimeBlockPhase()}});setupBackupActionButtons();setupSafeUpdateFlow();createRecoverySnapshot("app opened").catch(showRecoveryError);
