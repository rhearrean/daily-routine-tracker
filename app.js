const HABITS_KEY = "dailyRoutineHabits.v5";
const COMPLETIONS_KEY = "dailyRoutineCompletions.v5";
const OLD_KEYS = [
...
function loadCompletions() {
  return safeParse(localStorage.getItem(COMPLETIONS_KEY), {});
}
...
function habitsDueOn(date) {
  return loadHabits().filter(habit => isHabitDue(habit, date));
}

function isDone(dateKey, habitId) {
  const completions = loadCompletions();
  return Boolean(completions[dateKey]?.[habitId]);
}
  ...
  dueHabits.forEach(habit => {
    ...
    button.innerHTML = `
      ...
      <span>
        ...
        <small>Tap when completed</small>
      </span>
    `;
...
function exportHabits() {
  ...
  if (navigator.clipboard) {
    navigator.clipboard.writeText(backupBox.value).catch(() => {});
  }
...
function importHabits() {
  ...
  if (!text) {
    backupMessage.textContent = "Paste your habit backup first.";
    return;
  }
  ...
  if (!Array.isArray(habits) || habits.length === 0) {
    backupMessage.textContent = "That backup does not look valid.";
    return;
  }

  const cleaned = habits
    .filter(habit => habit.name && habit.schedule)
    .map(habit => ({
  ...
  if (cleaned.length === 0) {
    backupMessage.textContent = "No valid habits found in that backup.";
    return;
  }
...
habitSchedule.addEventListener("change", () => {
  customDays.classList.toggle("hidden", habitSchedule.value !== "custom");
});
...
habitForm.addEventListener("submit", event => {
  ...
  if (schedule === "custom" && selectedCustomDays.length === 0) {
    alert("Choose at least one custom day.");
    return;
  }
  ...
  customDays.querySelectorAll("button").forEach(button => button.classList.remove("selected"));
