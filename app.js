const APP_META={version:"10.8.8",build:"2026.07.17.routine-steps",schemaVersion:7,releaseDate:"July 17, 2026",releaseNotes:["Added optional Routine Steps to habits.","Each time-block occurrence has its own checklist.","Step checkmarks reset for every occurrence and every new day.","Parent habit completion remains manual."]};
...
const $=id=>document.getElementById(id);let selectedCustomDays=[],selectedOccurrenceBlocks=[],selectedRoutineSteps=[],editingHabitId=null,activeAction=null,expandedCompletedBlocks={},expandedRoutineSteps={};
...
if("serviceWorker"in navigator)navigator.serviceWorker.register("service-worker.js");setupSnoozeField();setupRoutineStepsField();migrateOldDataOnce();formatDateLabel();resetFormMode();render();setupBackupActionButtons();
