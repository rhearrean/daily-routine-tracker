const APP_META={version:"11.1.2",build:"2026.08.22.locked-repeat-logs",schemaVersion:7,releaseDate:"August 22, 2026",releaseNotes:["Uses one clear completion control for each repeatable habit.","Habits with steps use Log 1 Time; habits without steps use Log Completion.","Removes the Today-screen undo control so logged repeats stay locked.","Includes extended update information to verify scrolling on iPhone.","No habit, history, settings, or data-schema changes."]};
...
function simplifyRepeatControls(){document.querySelectorAll(".repeat-undo-btn,.has-routine-steps.repeatable-habit .repeat-add-btn").forEach(button=>button.remove());document.querySelectorAll(".repeatable-habit:not(.has-routine-steps) .habit-button").forEach(button=>{button.tabIndex=-1;button.setAttribute("aria-disabled","true")});document.querySelectorAll(".repeat-add-btn").forEach(button=>button.textContent="Log Completion");document.querySelectorAll(".repeat-actions").forEach(actions=>{if(!actions.children.length)actions.remove()})}
