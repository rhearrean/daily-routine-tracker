const APP_META={version:"11.1.3",build:"2026.08.22.lock-repeat-target",schemaVersion:7,releaseDate:"August 22, 2026",releaseNotes:["Locks a repeatable habit after its optional target is reached.","The completed habit stays checked and its log button changes to Target Complete.","Open-ended repeatable habits without a target remain available for additional logs.","No habit, history, settings, or data-schema changes."]};
...
function simplifyRepeatControls(){document.querySelectorAll(".repeat-undo-btn,.has-routine-steps.repeatable-habit .repeat-add-btn").forEach(button=>button.remove());document.querySelectorAll(".repeatable-habit:not(.has-routine-steps) .habit-button").forEach(button=>{button.tabIndex=-1;button.setAttribute("aria-disabled","true")});document.querySelectorAll(".repeat-add-btn").forEach(button=>button.textContent="Log Completion");document.querySelectorAll(".repeatable-habit").forEach(card=>{const targetReached=card.querySelector(".habit-button.done .repeat-count-pill")?.textContent.includes("/");if(!targetReached)return;const logButton=card.querySelector(".repeat-add-btn,.routine-complete-btn");if(logButton){logButton.disabled=true;logButton.textContent="Target Complete"}});document.querySelectorAll(".repeat-actions").forEach(actions=>{if(!actions.children.length)actions.remove()})}
