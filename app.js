const APP_META={version:"10.8",build:"2026.07.01.today-focus",schemaVersion:6,releaseDate:"June 28, 2026",releaseNotes:["Today Focus UI: current block uses a green badge and stays expanded.","Future and completed blocks collapse to compact one-line summaries.","Stats UI hidden for now; no storage, import/export, streak, or history changes."]};
...
function blockSummaryText(completed,skipped,eligible){const parts=[];if(completed)parts.push(`${completed} completed`);if(skipped)parts.push(`${skipped} skipped`);if(!parts.length&&eligible===0)parts.push("All skipped");return parts.join(" • ")}function renderTimeBlocks(){
  E.timeBlocks.innerHTML="";
  const due=dueOccurrencesOn(new Date());
  const k=getTodayKey();
  E.emptyTodayText.classList.toggle("hidden",due.length>0);

  orderedTodayBlocks().forEach(block=>{
    const items=due.filter(i=>i.occurrence.block===block.id);
    if(items.length===0)return;

    const completed=items.filter(i=>isDone(k,i.occurrence.id)).length;
    const skipped=items.filter(i=>isSkipped(k,i.occurrence.id)).length;
    const eligible=items.length-skipped;
    const isAnytime=block.id==="anytime";
    const isCurrent=isCurrentTimedBlock(block);
    const isComplete=eligible>0&&completed>=eligible;
    const shouldCollapse=!isAnytime&&!isCurrent;
    const summary=compactBlockSummary(completed,eligible,skipped);

    const w=document.createElement("div");
    w.className=`time-block ${isAnytime?"anytime-block":""} ${isCurrent?"current-block":""} ${isComplete?"done-block":""} ${shouldCollapse?"compact-block":""}`;
    w.innerHTML=`<div class="time-block-header compact-header">
      <div class="block-title-line">
        <h3>${block.label}</h3>
        ${isCurrent?`<span class="current-badge"><span class="current-dot"></span>Current</span>`:""}
      </div>
      <span class="block-progress">${summary}</span>
    </div>
    ${shouldCollapse?"":`<small class="block-time">${blockTimeLabel(block)}</small><div class="habit-list"></div>`}`;

    if(shouldCollapse){
      w.addEventListener("click",()=>{
        if(!w.classList.contains("compact-block"))return;
        w.classList.remove("compact-block");
        const time=document.createElement("small");
        time.className="block-time";
        time.textContent=blockTimeLabel(block);
        const habitList=document.createElement("div");
        habitList.className="habit-list";
        renderHabitButtons(habitList,items,k);
        w.appendChild(time);
        w.appendChild(habitList);
      });
      E.timeBlocks.appendChild(w);
      return;
    }

    renderHabitButtons(w.querySelector(".habit-list"),items,k);
    E.timeBlocks.appendChild(w);
  });
}

function renderHabitButtons(list,items,k){
  if(!list)return;
  items.forEach(({habit,occurrence})=>{
    const done=isDone(k,occurrence.id);
    const skipped=isSkipped(k,occurrence.id);
    const status=completionStatus(k,occurrence.id);
    const locked=!done&&!skipped&&habit.allowLate===false&&isPastCutoff(habit);
    const b=document.createElement("button");
    b.className=`habit-button ${done?(status==="late"?"late":"done"):""} ${skipped?"skipped":""} ${locked?"locked":""}`;
    b.type="button";
    const sorted=sortOccurrences(habit.occurrences);
    const num=habit.occurrences.length>1?` (${sorted.findIndex(o=>o.id===occurrence.id)+1} of ${habit.occurrences.length})`:"";
    const targetLine=habit.cutoff?`<small>Target: ${formatTime(habit.cutoff)}</small>`:"";
    const pill=skipped?`<span class="status-pill skipped">Skipped</span>`:done?`<span class="status-pill ${status==="late"?"late":"on-time"}">${status==="late"?"Late":"On time"}</span>`:locked?`<span class="status-pill locked">Missed cutoff</span>`:habit.cutoff?`<span class="status-pill">Target: ${formatTime(habit.cutoff)}</span>`:`<span class="status-pill">Tap to complete • Hold for options</span>`;
    b.innerHTML=`<span class="checkbox" aria-hidden="true"></span><span><strong>${habit.name}${num}</strong>${targetLine}${pill}</span>`;
    b.addEventListener("click",()=>toggleDone(k,habit,occurrence));
    b.addEventListener("contextmenu",e=>{e.preventDefault();openActionMenu(k,habit,occurrence)});
    b.addEventListener("touchstart",()=>{b._pressTimer=setTimeout(()=>openActionMenu(k,habit,occurrence),550)},{passive:true});
    b.addEventListener("touchend",()=>clearTimeout(b._pressTimer));
    b.addEventListener("touchmove",()=>clearTimeout(b._pressTimer));
    list.appendChild(b);
  });
}
