const APP_META={version:"10.8.1",build:"2026.07.04.current-badge-render-fix",schemaVersion:6,releaseDate:"June 28, 2026",releaseNotes:["Fixed Current badge rendering by applying it after time blocks render.","No storage, import/export, habits, or history changes."]};
...
function blockSummaryText(completed,skipped,eligible){const parts=[];if(completed)parts.push(`${completed} completed`);if(skipped)parts.push(`${skipped} skipped`);if(!parts.length&&eligible===0)parts.push("All skipped");return parts.join(" • ")}function renderTimeBlocks(){E.timeBlocks.innerHTML="";const due=dueOccurrencesOn(new Date()),k=getTodayKey(),settings=loadSettings();E.emptyTodayText.classList.toggle("hidden",due.length>0);orderedTodayBlocks().forEach(block=>{const items=due.filter(i=>i.occurrence.block===block.id);if(!items.length)return;const completed=items.filter(i=>isDone(k,i.occurrence.id)).length,skipped=items.filter(i=>isSkipped(k,i.occurrence.id)).length,eligible=items.length-skipped,remaining=Math.max(eligible-completed,0),blockComplete=items.length>0&&remaining===0,autoCollapse=settings.autoCollapseCompletedBlocks!==false,collapsed=autoCollapse&&blockComplete&&!expandedCompletedBlocks[block.id],w=document.createElement("div");w.className=`time-block ${collapsed?"collapsed":""}`;const summary=blockComplete?blockSummaryText(completed,skipped,eligible):`${remaining} remaining`;w.innerHTML=`<div class="time-block-header"><div><h3>${collapsed?"▶":"▼"} ${block.label}${blockComplete?" ✓":""}</h3><small>${blockTimeLabel(block)}</small>${collapsed?`<div class="block-summary">${summary}</div>`:""}</div><span class="block-progress">${blockComplete?summary:`${completed}/${eligible}${skipped?` • ${skipped} skipped`:""}`}</span></div><div class="habit-list"></div>`;w.querySelector(".time-block-header").addEventListener("click",e=>{if(e.target.closest("button"))return;if(blockComplete){expandedCompletedBlocks[block.id]=collapsed;render()}});const list=w.querySelector(".habit-list");items.forEach(({habit,occurrence})=>{const b=document.createElement("button"),done=isDone(k,occurrence.id),skipped=isSkipped(k,occurrence.id),status=completionStatus(k,occurrence.id),locked=!done&&!skipped&&habit.allowLate===false&&isPastCutoff(habit);b.className=`habit-button ${done?(status==="late"?"late":"done"):""} ${skipped?"skipped":""} ${locked?"locked":""}`;b.type="button";const sorted=sortOccurrences(habit.occurrences),num=habit.occurrences.length>1?` (${sorted.findIndex(o=>o.id===occurrence.id)+1} of ${habit.occurrences.length})`:"";const pill=skipped?`<span class="status-pill skipped">Skipped</span>`:done?`<span class="status-pill ${status==="late"?"late":"on-time"}">${status==="late"?"Late":"On time"}</span>`:locked?`<span class="status-pill locked">Missed cutoff</span>`:habit.cutoff?`<span class="status-pill">Target: ${formatTime(habit.cutoff)}</span>`:`<span class="status-pill">Tap complete • hold options</span>`;b.innerHTML=`<span class="checkbox"></span><span><strong>${habit.name}${num}</strong><small>${habit.cutoff?`Target: ${formatTime(habit.cutoff)}`:""}</small>${pill}</span>`;b.addEventListener("click",e=>{e.stopPropagation();toggleDone(k,habit,occurrence)});b.addEventListener("contextmenu",e=>{e.preventDefault();openActionMenu(k,habit,occurrence)});b.addEventListener("touchstart",()=>{b._pressTimer=setTimeout(()=>openActionMenu(k,habit,occurrence),550)},{passive:true});b.addEventListener("touchend",()=>clearTimeout(b._pressTimer));b.addEventListener("touchmove",()=>clearTimeout(b._pressTimer));list.appendChild(b)});E.timeBlocks.appendChild(w)})}
...
function applyCurrentBadge(){
  try{
    const current=loadBlocks().find(block=>isCurrentTimedBlock(block));
    if(!current)return;
    document.querySelectorAll(".current-badge").forEach(el=>el.remove());
    const blocks=[...document.querySelectorAll(".time-block")];
    const target=blocks.find(card=>{
      const h3=card.querySelector(".time-block-header h3");
      return h3&&h3.textContent.trim().startsWith(current.label.trim());
    });
    if(!target)return;
    target.classList.add("current-block");
    const small=target.querySelector(".time-block-header small");
    const h3=target.querySelector(".time-block-header h3");
    const badge=document.createElement("span");
    badge.className="current-badge";
    badge.innerHTML='<span class="current-dot"></span>Current';
    if(small)small.appendChild(badge);
    else if(h3)h3.appendChild(badge);
  }catch(e){
    console.warn("Current badge skipped",e);
  }
}

function render(){const p=getDailyProgress(new Date());renderTimeBlocks();E.progressText.textContent=`${p.completed}/${p.eligible}${p.skipped?` (${p.skipped} skipped)`:""}`;E.progressPercent.textContent=`${p.percent}%`;document.documentElement.style.setProperty("--progress",p.percent);const streak=getOverallCurrentStreak();E.streakText.textContent=`${streak} ${streak===1?"day":"days"}`;const week=getWeekCount();E.weekText.textContent=`${week.completed}/${week.daysWithHabits||7}`;renderRecentDays();renderAllHabits();renderHabitStats();renderBlockSettings();renderOccurrenceButtons();renderDisplaySettings();renderAppInfo();applyCurrentBadge()}
...
setTimeout(applyCurrentBadge,150);
if("serviceWorker"in navigator)navigator.serviceWorker.register("service-worker.js");migrateOldDataOnce();formatDateLabel();resetFormMode();render();setupBackupActionButtons();
