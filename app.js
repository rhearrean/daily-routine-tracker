const APP_META={version:"10.8.1",build:"2026.06.29.2",schemaVersion:6,releaseDate:"June 28, 2026",releaseNotes:["Emergency hotfix for v10.8 reload/data persistence issue.","Defined overdue prompt storage key used after reload.","Added safer storage fallbacks for habits and completions."]};
const HABITS_KEY="dailyRoutineHabits.v10_1",COMPLETIONS_KEY="dailyRoutineCompletions.v10_1",BLOCKS_KEY="dailyRoutineTimeBlocks.v10_1",SETTINGS_KEY="dailyRoutineSettings.v10_3",OVERDUE_KEY="dailyRoutineOverdueDismissals.v10_8";
...
const $=id=>document.getElementById(id);let selectedCustomDays=[],selectedOccurrenceBlocks=[],editingHabitId=null,activeAction=null,expandedCompletedBlocks={};
const E={todayTitle:$("todayTitle"),dateText:$("dateText"),mainVersionText:$("mainVersionText"),progressText:$("progressText"),progressPercent:$("progressPercent"),timeBlocks:$("timeBlocks"),emptyTodayText:$("emptyTodayText"),resetTodayBtn:$("resetTodayBtn"),streakText:$("streakText"),weekText:$("weekText"),recentDays:$("recentDays"),habitForm:$("habitForm"),habitName:$("habitName"),habitSchedule:$("habitSchedule"),customDays:$("customDays"),allHabits:$("allHabits"),habitStats:$("habitStats"),openAddHabitBtn:$("openAddHabitBtn"),openStatsBtn:$("openStatsBtn"),closeStatsBtn:$("closeStatsBtn"),statsPanel:$("statsPanel"),openSettingsBtn:$("openSettingsBtn"),closeSettingsBtn:$("closeSettingsBtn"),habitEditorSheet:$("habitEditorSheet"),closeHabitEditorBtn:$("closeHabitEditorBtn"),settingsPanel:$("settingsPanel"),exportBtn:$("exportBtn"),importBtn:$("importBtn"),backupBox:$("backupBox"),backupMessage:$("backupMessage"),formTitle:$("formTitle"),formModeLabel:$("formModeLabel"),saveHabitBtn:$("saveHabitBtn"),cancelEditBtn:$("cancelEditBtn"),habitCutoff:$("habitCutoff"),habitAllowLate:$("habitAllowLate"),occurrenceBlocks:$("occurrenceBlocks"),blockSettings:$("blockSettings"),saveBlocksBtn:$("saveBlocksBtn"),appInfo:$("appInfo"),actionSheet:$("actionSheet"),actionTitle:$("actionTitle"),actionCompleteBtn:$("actionCompleteBtn"),actionSkipBtn:$("actionSkipBtn"),actionClearBtn:$("actionClearBtn"),actionCancelBtn:$("actionCancelBtn"),autoCollapseBlocks:$("autoCollapseBlocks")};
function safeParse(v,f){try{return JSON.parse(v)||f}catch{return f}}
function safeGetStorage(key,fallback=null){try{return localStorage.getItem(key)??fallback}catch{return fallback}}
function safeSetStorage(key,value){try{localStorage.setItem(key,value);return true}catch{return false}}function getLocalDateKey(d=new Date()){return`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`}function dateFromOffset(o=0){const d=new Date();d.setHours(12,0,0,0);d.setDate(d.getDate()+o);return d}function getTodayKey(){return getLocalDateKey(new Date())}function nowMinutes(){const d=new Date();return d.getHours()*60+d.getMinutes()}function timeToMinutes(v){if(!v)return null;const[h,m]=v.split(":").map(Number);return h*60+m}function formatTime(v){if(!v)return"";const[h,m]=v.split(":").map(Number),d=new Date();d.setHours(h,m,0,0);return d.toLocaleTimeString(undefined,{hour:"numeric",minute:"2-digit"})}function blockTimeLabel(b){return b.start&&b.end?`${formatTime(b.start)} – ${formatTime(b.end)}`:"Anytime"}function makeId(name){const slug=String(name||"habit").trim().toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"");return`${slug||"habit"}-${Date.now()}`}
function loadSettings(){return{autoCollapseCompletedBlocks:true,...safeParse(safeGetStorage(SETTINGS_KEY),{})}}function saveSettings(settings){safeSetStorage(SETTINGS_KEY,JSON.stringify(settings))}
function loadBlocks(){const s=safeParse(safeGetStorage(BLOCKS_KEY),null);if(!Array.isArray(s)||s.length===0){saveBlocks(DEFAULT_BLOCKS);return DEFAULT_BLOCKS}return DEFAULT_BLOCKS.map(d=>({...d,...(s.find(b=>b.id===d.id)||{})}))}function saveBlocks(b){safeSetStorage(BLOCKS_KEY,JSON.stringify(b))}function blockOrderMap(){const m={};loadBlocks().forEach((b,i)=>m[b.id]=i);return m}function sortOccurrences(occ){const o=blockOrderMap();return[...occ].sort((a,b)=>(o[a.block]??999)-(o[b.block]??999))}
function orderedTodayBlocks(){
...
function blockEndMinutes(block){return block&&block.end?blockMinutes(block.end):null}
function todayOverdueDismissals(){
  const saved=safeParse(safeGetStorage(OVERDUE_KEY),{});
  return saved[getTodayKey()]||{};
}
function saveTodayOverdueDismissal(blockId,mode){
  const saved=safeParse(safeGetStorage(OVERDUE_KEY),{});
  const today=getTodayKey();
  saved[today]=saved[today]||{};
  saved[today][blockId]={mode,until:Date.now()+(mode==="later"?30*60*1000:24*60*60*1000)};
  safeSetStorage(OVERDUE_KEY,JSON.stringify(saved));
}
function isOverdueDismissed(blockId){
  const d=todayOverdueDismissals()[blockId];
  return d&&d.until>Date.now();
}
function getOverdueBlock(){
  const grace=30;
  const now=currentMinutes();
  const key=getTodayKey();
  const due=dueOccurrencesOn(new Date());
  for(const block of loadBlocks()){
    if(block.id==="anytime"||!block.end||isOverdueDismissed(block.id))continue;
    const end=blockEndMinutes(block);
    if(end===null||now<=end+grace)continue;
    const items=due.filter(item=>item.occurrence.block===block.id);
    const pending=items.filter(item=>!isResolved(key,item.occurrence.id));
    if(pending.length>0)return{block,pending};
  }
  return null;
}
function openOverduePrompt(data){
  if(!data||!overdueSheet)return;
  activeOverdue=data;
  overdueTitle.textContent=`${data.block.label.replace(/^[^A-Za-z0-9]+\\s*/,"")} is overdue`;
  overdueText.textContent=`${data.pending.length} unfinished habit${data.pending.length===1?"":"s"} remain. Skip them and keep the day moving?`;
  overdueSheet.classList.remove("hidden");
}
function closeOverduePrompt(){activeOverdue=null;if(overdueSheet)overdueSheet.classList.add("hidden")}
function checkOverdueBlocks(){
  if(activeOverdue)return;
  const data=getOverdueBlock();
  if(data)openOverduePrompt(data);
}
function safeCheckOverdueBlocks(){
  try{checkOverdueBlocks()}catch(e){console.warn("Overdue check skipped",e)}
}
...
function normalizeCompletionEntry(e){if(e===true)return{state:"done",done:true,status:"done",completedAt:""};if(e&&typeof e==="object"){const state=e.state||(e.skipped?"skipped":(e.done!==false?"done":"pending"));return{...e,state,done:state==="done",status:e.status||state}}return null}function normalizeHabit(h){const oldBlock=h.block||"anytime";let occ=Array.isArray(h.occurrences)&&h.occurrences.length>0?h.occurrences:[{id:`${h.id||makeId(h.name||"habit")}-${oldBlock}`,block:oldBlock}];occ=occ.map((o,i)=>({id:o.id||`${h.id||makeId(h.name||"habit")}-${o.block||oldBlock}-${i}`,block:o.block||oldBlock}));return{id:h.id||makeId(h.name||"Habit"),name:h.name||"Habit",schedule:h.schedule||"daily",days:Array.isArray(h.days)?h.days:[],occurrences:sortOccurrences(occ),cutoff:h.cutoff||"",allowLate:h.allowLate!==false}}
function migrateOldDataOnce(){if(!safeGetStorage(BLOCKS_KEY)){for(const[,,bk]of OLD_KEYS){if(bk){const ob=safeParse(localStorage.getItem(bk),null);if(Array.isArray(ob)&&ob.length>0){saveBlocks(ob);break}}}if(!safeGetStorage(BLOCKS_KEY))saveBlocks(DEFAULT_BLOCKS)}if(!safeGetStorage(HABITS_KEY)){for(const[hk]of OLD_KEYS){const oh=safeParse(localStorage.getItem(hk),null);if(Array.isArray(oh)&&oh.length>0){saveHabits(oh.map(normalizeHabit));break}}}if(!safeGetStorage(COMPLETIONS_KEY)){for(const[,ck]of OLD_KEYS){const old=safeParse(localStorage.getItem(ck),null);if(old&&typeof old==="object"){const habits=loadHabits(),converted={};Object.keys(old).forEach(dateKey=>{converted[dateKey]={};Object.keys(old[dateKey]||{}).forEach(key=>{const habit=habits.find(h=>h.id===key);if(habit)converted[dateKey][habit.occurrences[0].id]=normalizeCompletionEntry(old[dateKey][key]);else converted[dateKey][key]=normalizeCompletionEntry(old[dateKey][key])})});saveCompletions(converted);break}}}}
function formatDateLabel(){const d=new Date();E.todayTitle.textContent=d.toLocaleDateString(undefined,{weekday:"long"});E.dateText.textContent=d.toLocaleDateString(undefined,{month:"long",day:"numeric",year:"numeric"});E.mainVersionText.textContent=`v${APP_META.version} • Build ${APP_META.build}`}function loadHabits(){const s=safeParse(safeGetStorage(HABITS_KEY),null);if(!s||!Array.isArray(s)||s.length===0){saveHabits(defaultHabits);return defaultHabits}return s.map(normalizeHabit)}function saveHabits(h){safeSetStorage(HABITS_KEY,JSON.stringify(h.map(normalizeHabit)))}function loadCompletions(){return safeParse(safeGetStorage(COMPLETIONS_KEY),{})}function saveCompletions(d){safeSetStorage(COMPLETIONS_KEY,JSON.stringify(d))}
...
function blockSummaryText(completed,skipped,eligible){const parts=[];if(completed)parts.push(`${completed} completed`);if(skipped)parts.push(`${skipped} skipped`);if(!parts.length&&eligible===0)parts.push("All skipped");return parts.join(" • ")}function renderTimeBlocks(){E.timeBlocks.innerHTML="";const due=dueOccurrencesOn(new Date()),k=getTodayKey(),settings=loadSettings();E.emptyTodayText.classList.toggle("hidden",due.length>0);orderedTodayBlocks().forEach(block=>{const items=due.filter(i=>i.occurrence.block===block.id);if(!items.length)return;const completed=items.filter(i=>isDone(k,i.occurrence.id)).length,skipped=items.filter(i=>isSkipped(k,i.occurrence.id)).length,eligible=items.length-skipped,remaining=Math.max(eligible-completed,0),blockComplete=items.length>0&&remaining===0,autoCollapse=settings.autoCollapseCompletedBlocks!==false,collapsed=autoCollapse&&blockComplete&&!expandedCompletedBlocks[block.id],w=document.createElement("div");w.className=`time-block ${collapsed?"collapsed":""}`;const summary=blockComplete?blockSummaryText(completed,skipped,eligible):`${remaining} remaining`;w.innerHTML=`<div class="time-block-header"><div><h3>${collapsed?"▶":"▼"} ${block.label}${blockComplete?" ✓":""}</h3><small>${blockTimeLabel(block)}</small>${collapsed?`<div class="block-summary">${summary}</div>`:""}</div><span class="block-progress">${blockComplete?summary:`${completed}/${eligible}${skipped?` • ${skipped} skipped`:""}`}</span></div><div class="habit-list"></div>`;w.querySelector(".time-block-header").addEventListener("click",e=>{if(e.target.closest("button"))return;if(blockComplete){expandedCompletedBlocks[block.id]=collapsed;render()}});const list=w.querySelector(".habit-list");items.forEach(({habit,occurrence})=>{const b=document.createElement("button"),done=isDone(k,occurrence.id),skipped=isSkipped(k,occurrence.id),status=completionStatus(k,occurrence.id),locked=!done&&!skipped&&habit.allowLate===false&&isPastCutoff(habit);b.className=`habit-button ${done?(status==="late"?"late":"done"):""} ${skipped?"skipped":""} ${locked?"locked":""}`;b.type="button";const sorted=sortOccurrences(habit.occurrences),num=habit.occurrences.length>1?` (${sorted.findIndex(o=>o.id===occurrence.id)+1} of ${habit.occurrences.length})`:"";const pill=skipped?`<span class="status-pill skipped">Skipped</span>`:done?`<span class="status-pill ${status==="late"?"late":"on-time"}">${status==="late"?"Late":"On time"}</span>`:locked?`<span class="status-pill locked">Missed cutoff</span>`:habit.cutoff?`<span class="status-pill">Target: ${formatTime(habit.cutoff)}</span>`:`<span class="status-pill">Tap complete • hold options</span>`;b.innerHTML=`<span class="checkbox"></span><span><strong>${habit.name}${num}</strong><small>${habit.cutoff?`Target: ${formatTime(habit.cutoff)}`:"No target time"}</small>${pill}</span>`;b.addEventListener("click",e=>{e.stopPropagation();toggleDone(k,habit,occurrence)});b.addEventListener("contextmenu",e=>{e.preventDefault();openActionMenu(k,habit,occurrence)});b.addEventListener("touchstart",()=>{b._pressTimer=setTimeout(()=>openActionMenu(k,habit,occurrence),550)},{passive:true});b.addEventListener("touchend",()=>clearTimeout(b._pressTimer));b.addEventListener("touchmove",()=>clearTimeout(b._pressTimer));list.appendChild(b)});E.timeBlocks.appendChild(w)})}
function renderRecentDays(){E.recentDays.innerHTML="";for(let o=-6;o<=0;o++){const d=dateFromOffset(o),p=getDailyProgress(d),el=document.createElement("div");el.className=`day-pill ${p.eligible>0&&p.completed===p.eligible?"done":""}`;el.textContent=d.toLocaleDateString(undefined,{weekday:"short"});E.recentDays.appendChild(el)}}function occurrenceLabel(h){return sortOccurrences(h.occurrences).map(o=>blockLabel(o.block).replace(/^[^\w]+ /,"")).join(", ")}function renderAllHabits(){E.allHabits.innerHTML="";const habits=loadHabits();if(!habits.length){E.allHabits.innerHTML=`<p class="empty-text">No habits yet. Add one above.</p>`;return}habits.forEach(h=>{const row=document.createElement("div");row.className="habit-row";row.innerHTML=`<div><strong>${h.name}</strong><small>${occurrenceLabel(h)} • ${scheduleLabel(h)} • ${h.cutoff?formatTime(h.cutoff):"No cutoff"}${h.allowLate?"":" • locks after cutoff"}</small></div><div class="habit-actions"><button class="edit-btn" type="button">Edit</button><button class="danger-btn" type="button">Delete</button></div>`;row.querySelector(".edit-btn").addEventListener("click",()=>startEditHabit(h.id));row.querySelector(".danger-btn").addEventListener("click",()=>{if(confirm(`Delete "${h.name}"?`))deleteHabit(h.id)});E.allHabits.appendChild(row)})}function renderHabitStats(){E.habitStats.innerHTML="";const habits=loadHabits();if(!habits.length){E.habitStats.innerHTML=`<p class="empty-text">No habit stats yet.</p>`;return}habits.forEach(h=>{const s=getHabitStats(h),card=document.createElement("div");card.className="habit-stat-card";card.innerHTML=`<strong>${s.name}</strong><small>${s.occurrences.map(o=>o.label).join(", ")} • ${s.schedule} • ${s.cutoff?formatTime(s.cutoff):"No cutoff"}</small><div class="metric-row"><div class="metric"><b>${s.currentStreak}</b><span>Current</span></div><div class="metric"><b>${s.longestStreak}</b><span>Longest</span></div><div class="metric"><b>${s.last30Days.percent}%</b><span>30 Days</span></div><div class="metric"><b>${s.totalSkipped}</b><span>Skipped</span></div><div class="metric"><b>${s.totalLate}</b><span>Late</span></div><div class="metric"><b>${s.totalCompleted}</b><span>Total</span></div></div>`;E.habitStats.appendChild(card)})}
function renderBlockSettings(){E.blockSettings.innerHTML="";loadBlocks().filter(b=>b.id!=="anytime").forEach(block=>{const row=document.createElement("div");row.className="block-setting-row";row.innerHTML=`<strong>${block.label}</strong><label>Start<input type="time" data-block="${block.id}" data-field="start" value="${block.start}"></label><label>End<input type="time" data-block="${block.id}" data-field="end" value="${block.end}"></label>`;E.blockSettings.appendChild(row)})}function renderOccurrenceButtons(){E.occurrenceBlocks.innerHTML="";loadBlocks().forEach(block=>{const b=document.createElement("button");b.type="button";b.dataset.block=block.id;b.className=selectedOccurrenceBlocks.includes(block.id)?"selected":"";b.textContent=block.label;b.addEventListener("click",()=>{selectedOccurrenceBlocks=selectedOccurrenceBlocks.includes(block.id)?selectedOccurrenceBlocks.filter(id=>id!==block.id):[...selectedOccurrenceBlocks,block.id];b.classList.toggle("selected",selectedOccurrenceBlocks.includes(block.id))});E.occurrenceBlocks.appendChild(b)})}function renderDisplaySettings(){if(E.autoCollapseBlocks)E.autoCollapseBlocks.checked=loadSettings().autoCollapseCompletedBlocks!==false}function renderAppInfo(){E.appInfo.innerHTML=`<div class="info-row"><strong>Version ${APP_META.version}</strong><small>Build ${APP_META.build}</small><small>Schema ${APP_META.schemaVersion}</small><small>Released ${APP_META.releaseDate}</small></div><div class="release-note"><strong>Release Notes</strong><ul>${APP_META.releaseNotes.map(n=>`<li>${n}</li>`).join("")}</ul></div>`}function render(){const p=getDailyProgress(new Date());renderTimeBlocks();E.progressText.textContent=`${p.completed}/${p.eligible}${p.skipped?` (${p.skipped} skipped)`:""}`;E.progressPercent.textContent=`${p.percent}%`;document.documentElement.style.setProperty("--progress",p.percent);const streak=getOverallCurrentStreak();E.streakText.textContent=`${streak} ${streak===1?"day":"days"}`;const week=getWeekCount();E.weekText.textContent=`${week.completed}/${week.daysWithHabits||7}`;renderRecentDays();renderAllHabits();renderHabitStats();renderBlockSettings();renderOccurrenceButtons();renderDisplaySettings();renderAppInfo();setTimeout(safeCheckOverdueBlocks,100)}
...
if(overdueSkipBtn){
  overdueSkipBtn.addEventListener("click",()=>{
    if(activeOverdue){
      const key=getTodayKey();
      activeOverdue.pending.forEach(({habit,occurrence})=>setOccurrenceState(key,habit,occurrence,"skipped"));
      saveTodayOverdueDismissal(activeOverdue.block.id,"skip");
      closeOverduePrompt();
    }
  });
}
if(overdueLaterBtn){
  overdueLaterBtn.addEventListener("click",()=>{
    if(activeOverdue){saveTodayOverdueDismissal(activeOverdue.block.id,"later");closeOverduePrompt();}
  });
}
if(overdueKeepBtn){
  overdueKeepBtn.addEventListener("click",()=>{
    if(activeOverdue){saveTodayOverdueDismissal(activeOverdue.block.id,"keep");closeOverduePrompt();}
  });
}
if(overdueSheet){
  overdueSheet.addEventListener("click",e=>{if(e.target===overdueSheet)closeOverduePrompt()});
}
