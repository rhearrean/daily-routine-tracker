const APP_META={version:"11.0.1",build:"2026.08.21.active-block-completion-fix",schemaVersion:7,releaseDate:"August 21, 2026",releaseNotes:["Fixes the end-of-day screen when a habit also belongs to a time block that is not active today.","Only occurrences in today's active time blocks now count toward Today and its completion screen.","No habit, routine, history, settings, or data-schema changes."]};
...
function setOccurrenceState(k,h,o,state){
  ...
  else if(state==="skipped")c[k][o.id]={state:"skipped",done:false,status:"skipped",skippedAt:new Date().toISOString(),habitId:h.id,occurrenceId:o.id,block:o.block};
  else{
