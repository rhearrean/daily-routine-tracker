...
setupSnoozeField();setupRoutineStepsField();migrateOldDataOnce();clearExpiredSnoozes();formatDateLabel();resetFormMode();render();lastTimeBlockPhaseSignature=timeBlockPhaseSignature();setInterval(refreshTimeBlockPhase,30000);document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="visible")refreshTimeBlockPhase()});setupBackupActionButtons();setupSafeUpdateFlow();createRecoverySnapshot("app opened").catch(showRecoveryError);
