const APP_META={version:"10.8.7",build:"2026.07.15.reorder-habits",schemaVersion:6,releaseDate:"June 28, 2026",releaseNotes:["Added habit reordering in Settings.","Use the up and down buttons to change habit order.","The saved order is used on Today and prepares the app for optional Complete in Order."]};
...
function renderAllHabits(){E.allHabits.innerHTML="";const habits=loadHabits();if(!habits.length){E.allHabits.innerHTML=`<p class="empty-text">No habits yet. Add one above.</p>`;return}habits.forEach((h,index)=>{const snoozed=isHabitSnoozed(h),row=document.createElement("div");row.className=`habit-row ${snoozed?"snoozed-habit":""}`;const status=snoozed?`<span class="snooze-pill">Snoozed until ${snoozeLabel(h)}</span>`:"";row.innerHTML=`<div class="habit-row-main"><strong>${h.name}</strong><small>${occurrenceLabel(h)} • ${scheduleLabel(h)} • ${h.cutoff?formatTime(h.cutoff):"No cutoff"}${h.allowLate?"":" • locks after cutoff"}${status?` • ${status}`:""}</small></div><div class="habit-actions"><div class="reorder-actions"><button class="reorder-btn move-up-btn" type="button" aria-label="Move ${h.name} up" ${index===0?"disabled":""}>↑</button><button class="reorder-btn move-down-btn" type="button" aria-label="Move ${h.name} down" ${index===habits.length-1?"disabled":""}>↓</button></div>${snoozed?`<button class="small-btn resume-btn" type="button">Resume</button>`:""}<button class="edit-btn" type="button">Edit</button><button class="danger-btn" type="button">Delete</button></div>`;row.querySelector(".move-up-btn").addEventListener("click",()=>moveHabit(h.id,-1));row.querySelector(".move-down-btn").addEventListener("click",()=>moveHabit(h.id,1));const resume=row.querySelector(".resume-btn");if(resume)resume.addEventListener("click",()=>resumeHabit(h.id));row.querySelector(".edit-btn").addEventListener("click",()=>startEditHabit(h.id));row.querySelector(".danger-btn").addEventListener("click",()=>{if(confirm(`Delete "${h.name}"?`))deleteHabit(h.id)});E.allHabits.appendChild(row)})}function renderHabitStats(){E.habitStats.innerHTML="";const habits=loadHabits();if(!habits.length){E.habitStats.innerHTML=`<p class="empty-text">No habit stats yet.</p>`;return}habits.forEach(h=>{const s=getHabitStats(h),card=document.createElement("div");card.className="habit-stat-card";card.innerHTML=`<strong>${s.name}</strong><small>${s.occurrences.map(o=>o.label).join(", ")} • ${s.schedule} • ${s.cutoff?formatTime(s.cutoff):"No cutoff"}</small><div class="metric-row"><div class="metric"><b>${s.currentStreak}</b><span>Current</span></div><div class="metric"><b>${s.longestStreak}</b><span>Longest</span></div><div class="metric"><b>${s.last30Days.percent}%</b><span>30 Days</span></div><div class="metric"><b>${s.totalSkipped}</b><span>Skipped</span></div><div class="metric"><b>${s.totalLate}</b><span>Late</span></div><div class="metric"><b>${s.totalCompleted}</b><span>Total</span></div></div>`;E.habitStats.appendChild(card)})}
...
if("serviceWorker"in navigator)navigator.serviceWorker.register("service-worker.js");setupSnoozeField();migrateOldDataOnce();formatDateLabel();resetFormMode();render();setupBackupActionButtons();
