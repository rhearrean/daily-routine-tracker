...
assert.match(app,/version:"10\.10\.10"/);
...
for(const asset of ["manifest.json","style.css","app.js"]){
  assert.match(html,new RegExp(`${asset.replace(".","\\.")}[?]v=10\\.10\\.10`));
}

assert.match(worker,/CACHE_NAME="daily-routine-v10-10-10"/);
...
assert.doesNotMatch(worker,/10\.10\.9/);

console.log("Safe update assertions passed for v10.10.10");
