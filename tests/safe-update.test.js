...
assert.match(app,/version:"10\.10\.11"/);
...
for(const asset of ["manifest.json","style.css","app.js"]){
  assert.match(html,new RegExp(`${asset.replace(".","\\.")}[?]v=10\\.10\\.11`));
}

assert.match(worker,/CACHE_NAME="daily-routine-v10-10-11"/);
...
assert.doesNotMatch(worker,/10\.10\.10/);

console.log("Safe update assertions passed for v10.10.11");
