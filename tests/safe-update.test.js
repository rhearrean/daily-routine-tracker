...
assert.match(app,/version:"11\.1\.6"/);
...
assert.match(app,/function blockScheduledOn/);
assert.match(app,/function activeTodayBlockSwitch/);
assert.match(app,/function applyTodayBlockSwitch/);
assert.match(app,/function clearTodayBlockSwitch/);
assert.match(app,/function clearExpiredTodayBlockSwitch/);
assert.match(app,/todayBlockSwitch:\{date:getTodayKey\(\),fromBlockId,toBlockId\}/);
...
assert.match(html,/id="todayBlockSwitchFrom"/);
assert.match(html,/id="todayBlockSwitchTo"/);
assert.match(html,/id="applyTodayBlockSwitchBtn"/);
assert.match(html,/id="clearTodayBlockSwitchBtn"/);
...
assert.match(style,/\.repeat-undo-btn\{display:none\}/);
assert.match(style,/\.has-routine-steps\.repeatable-habit \.repeat-add-btn\{display:none\}/);
assert.match(style,/content:"Log Completion"/);
assert.match(style,/\.today-block-switch\{/);

for(const asset of ["manifest.json","style.css","app.js"]){
  assert.match(html,new RegExp(`${asset.replace(".","\\.")}[?]v=11\\.1\\.6`));
}

assert.match(worker,/CACHE_NAME="daily-routine-v11-1-6"/);
assert.match(worker,/const RELEASE_META=\{version:"11\.1\.6"/);
assert.match(worker,/Today-only switch between two time blocks/);
...
assert.doesNotMatch(worker,/11\.1\.5/);

console.log("Safe update assertions passed for v11.1.6");
